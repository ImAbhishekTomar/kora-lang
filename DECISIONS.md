...
- **`x: T = <outcome> else:` binds the payload or leaves.** Every model call
  ...
  - It deliberately collapses `Uncertain` and `Exhausted`. That is a real loss:
    `Exhausted` means the budget ran out, not that the model declined. It is
    accepted because both are handled identically in the code this exists to
    flatten, and because the outcome is still recorded — a swallowed
    `Exhausted` is not an invisible one. When the difference matters, `match`
    is still there.
