...
git-ignored. One line per effect, appended and flushed as the run goes, so
the cost of `--durable` stays flat as a run gets long. A run written by an
older Kora, as one `.json` object, is converted the first time it is resumed
or answered.
...
so a resume can tell "never started" from "started, outcome unknown". What a
resume does with one depends on what was at stake:
...
something outside the program already happened. Start a new run once you have
checked, rather than resuming that one.
...
A live, annotated look at this — the banner, the color legend, real captured
runs — is at [Kora CLI, Dressed](https://claude.ai/code/artifact/41e03bfc-33f2-4f91-a35b-130e6ccecc7b).
