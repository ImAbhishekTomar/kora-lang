...
check`. Static errors and an invalid `kora.toml` stop the command instead of
running with ambiguous code or policy.
...
The checker covers local declared assignments and returns, function and
constructor arity, declared fields, duplicate definitions, loop-control
placement, and direct classified flow into `analyze`. Dynamic sidecars, MCP,
helpers, and untyped values remain runtime boundaries.

Exits non-zero if anything fails to parse or check.
...
If a file exists but cannot be read or parsed, commands that execute or
