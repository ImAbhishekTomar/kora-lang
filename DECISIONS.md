...
### Time is a budget meter, not a separate cancellation concept

The language already had a lexical fence that nests, only ever tightens, is
shared across a `parallel for`, and reports exhaustion as a value rather than
an exception. A deadline wants every one of those properties. Adding
`max_seconds` to `budget` gets them by construction; a separate `deadline` or
a cancellation-token API would have restated them, and then had to be argued
about separately when the two disagreed.

The deadline is stored as the instant it expires, not as a duration. Every
worker in a fan-out is then measured against one moment, instead of each
starting its own clock when its thread happens to begin — which is what
"thirty seconds" means to the person who wrote it.

Time is reported ahead of the other meters when several have run out. A scope
...
be recomputed. A clock cannot: a replay runs faster than the original, so
re-deciding would let the same run answer differently the second time — and
worse, send a call the first run refused. The refusal is written into the
call's own journal slot and replayed like any other outcome. It occupies that
slot rather than skipping it, so the effects after it keep their positions.

This is also why the budget check now runs *after* the journal lookup rather
...
| `xml` | a `DOCTYPE` entity can name a local file or an internal URL, so the parser becomes a file reader and an HTTP client (XXE) -- Python needed a separate library, `defusedxml`, because the standard one's defaults could not change; `ElementTree` reports a tag as `{uri}name`, so code strips the braces and matches the wrong namespace; character data is split between `.text` and `.tail`, so the obvious read of `<p>Hello <b>world</b>!</p>` loses two thirds of it; `xmltodict` gives one child as an object and two as a list | a `DOCTYPE` is refused outright, which takes the billion-laughs bomb with it since that needs one too; a namespace is a field and lookups match the local name; `text` is all of an element's character data in document order; `children` is always a list, at any length |
...
- Native stdlib: `json`, `yaml`, `xml`, `fs`, `time`, `re`, `http`, `csv`,
  `sql`, `env`, `notes`, `pdf` — **done**
