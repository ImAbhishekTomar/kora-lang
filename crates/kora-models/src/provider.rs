...
use crate::{AnalyzeOutcome, AnalyzeRequest, ModelConfig, ModelError, Provider};
...
fn openai(
    ...
    let body = json!({
        ...
        "messages": [
            {"role": "system", "content": system_prompt(&req.schema)},
            {"role": "user", "content": user_prompt(&req.prompt, &req.data_json)},
        ],
        "response_format": {
            "type": "json_schema",
            "json_schema": {
    ...
    parse_response(content, &req.schema, tokens_in, tokens_out)
}
...
mod tests {
    ...
    fn openai_request_shape_and_parse() {
        ...
        let outcome = analyze_with(&config, &request(), &*transport).unwrap();
        match outcome {
            AnalyzeOutcome::Ok {
                fields_json,
                tokens_in,
    ...
    fn openai_missing_key_is_clear() {
        ...
        let err = analyze_with(&config, &request(), &*transport).unwrap_err();
        assert!(
    ...
    fn ollama_request_shape_and_uncertain() {
        ...
        match analyze_with(&config, &request(), &*transport).unwrap() {
    ...
    fn ollama_endpoint_override() {
        ...
        analyze_with(&config, &request(), &*transport).unwrap();
        assert_eq!(
